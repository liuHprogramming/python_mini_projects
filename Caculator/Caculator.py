...
def get_input(entry, argu):
    entry.insert(END, argu)  # Eingabe am Ende des Texts addieren


def backspace(entry):
    input_len = len(entry.get())
    entry.delete(input_len-1)   # den letzte Symbol loeschen


def clear(entry):
    entry.delete(0, END)    # alle Eingaben loeschen
...
def cal():
    ...
    entry = Entry(root, justify="right", font=entry_font)   # Text eingeben
    entry.grid(row=0, column=0, columnspan=4, sticky=N+W+S+E, padx=5, pady=5)   # Columnspan=4 : 4 mal breiter
    ...
    # mit partial koennen wir wenige Parameter eingeben
    ...
    button7 = num_button(text='7', command=lambda: get_input(entry, '7'))   # create Button
    button7.grid(row=2, column=0, pady=5)   # decide the location
    ...
    button0.grid(row=5, column=0, columnspan=2, padx=3, pady=5, sticky=N + S + W + E)
    ...
    button15 = Button(root, text='<-', bg=other_button_bg, padx=10, pady=3, command=lambda: backspace(entry),
                      activebackground=button_active_bg)
    button15.grid(row=1, column=0, columnspan=2, padx=5, pady=5, sticky=N + S + W + E)

    button16 = Button(root, text='C', bg=other_button_bg, padx=10, pady=3, command=lambda: clear(entry),
                      activebackground=button_active_bg)
    button16.grid(row=1, column=2, pady=5)

    button17 = Button(root, text='=', bg=cal_button_bg, fg=text_fg, padx=10, pady=3, command=lambda: calc(entry),
                      activebackground=button_active_bg)
